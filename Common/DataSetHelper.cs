        ...
        private DataTable CreateGroupByTable(string tableName, DataTable sourceTable, string fieldList)
        {
            ...
            else
            {
                ...
                foreach (FieldInfo Field in GroupByFieldInfo)
                {
                    ...
                    {
                        // 为字段的类型设置值 2011-8-15 by David
                        Type ty = dc.DataType;
                        if (Field.Aggregate.ToLower() == "count")
                        {
                            ty=System.Type.GetType("System.Int32");
                            //dc.DataType = 
                        }
                        
                        dt.Columns.Add(Field.FieldAlias, ty);
                    }
                }
                if (ds != null)
                {
            ...
            foreach (DataRow sourceRow in rows)
            {
                ...
                if (lastSourceRow != null)
                {
                    ...
                    foreach (FieldInfo Field in m_FieldInfo)
                    {
                        ...
                        if (lastSourceRow[Field.FieldName].ToString().ToLower()!= sourceRow[Field.FieldName].ToString().ToLower())
                        {
                            sameRow = false;
                            break;
                        }
