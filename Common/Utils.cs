...
namespace Maticsoft.Common
{
    ...
    public class Utils
    {
        ...
        /// <param name="str"></param>
        /// <returns></returns>
        /// <remarks>2011-8-16 by David</remarks>
        public static string GetUnicode(string str)
        {
            ...
            foreach (char ch in str)
            {
                ...
                {
                    byte[] buffte = System.Text.Encoding.Unicode.GetBytes(new char[] { ch });
                    sb.AppendFormat("\\u{1:X2}{0:X2}", buffte[0], buffte[1]);
                }
            }
            return sb.ToString();
        }
        ...
        public static string cutstr(string str, int len)
        {
            ...
            {
                if ((int)Convert.ToChar(str.Substring(i, 1)) > 255 || (int)Convert.ToChar(str.Substring(i, 1)) < 0)
                {
                    tempLen += 2;
                }
                else
        ...
        /// <summary>
        /// 根据字符串返回日期值,出错时返回当前时间

        /// </summary>
        /// <param name="s"></param>
        /// <returns></returns>
        /// <remarks>2011-7-16 by David</remarks>
        public static DateTime GetDateTimeFromString(string s)
        {
            ...
            #region 月 或 日 不存在时的解决方法

            switch (CountCharInString(s, '-'))
            {
                case 0:
                    s += "-1-1";
                    break;
        ...
        /// <returns></returns>
        public static string OfferDays(DateTime endTime)
        {
            return OfferDays(DateTime.Now, endTime);
        }
        ...
        /// <returns></returns>
        /// <remarks>2011-7-16 by David</remarks>
        public static string OfferDays(DateTime startTime, DateTime endTime)
        {
            string resultString = "";
            //返回结果字符串
            ...
            //当前时间
            int days = startTime.Subtract(endTime).Days;  //时间差天数
        ...
        ///使实际天数减1
        ///</summary>
        ///<param name="strNormalDays">正常距当前的时间差字符串</param>
        ///<param name="endTime">要缩短的天数</param>
        public static string DayCutOne(string strNormalDays, int stepDays)
        {
            string strResult = "";
            if (strNormalDays == "今天")
            { }
            else if (strNormalDays == "昨天")
            {
                strResult = "今天";
            }
            else if (strNormalDays == "前天")
            {
                strResult = "昨天";
            }
            else
            {
                string strDay = strNormalDays.Substring(0, strNormalDays.IndexOf("天")).Trim();
                int currentDays = Convert.ToInt32(strDay);
                strResult = (currentDays > stepDays) ? currentDays - stepDays + "天前" : strResult;
                if (strResult == "1天前")
                    strResult = "昨天";
                if (strResult == "2天前")
                    strResult = "前天";
        ...
        /// 获取当前日期的差别天数 
        /// </summary>
        /// <param name="strdatetTme"></param>
        /// <returns></returns>
        public static string OfferDaysToday(string strdatetTme)
        {
            DateTime endTime = GetDateTimeFromString(strdatetTme);
            return OfferDays(endTime);
        }
        ...
        /// <param name="strName">名称</param>
        /// <param name="strValue">值</param>
        public static void WriteCookie(string strName, string key, string strValue, int days)
        {
            HttpCookie cookie = HttpContext.Current.Request.Cookies[strName];
            if (cookie == null)
            {
        ...
        public static string ConvertSimpleFileName(string fullname, string repstring, int leftnum, int rightnum, int charnum)
        {
            ...
            string extname = GetExtension(fullname);
        ...
        {
            if (HttpContext.Current == null) return ""; //在非web引用时返回串
        ...
        /// 得到选中的id
        /// </summary>
        /// <param name="repCtrl">传入的wReapter,必须包含input列,必须是checkbox,且名字为chkDelete</param>
        /// <remarks>2010-08-10 by David</remarks>
        /// <returns></returns>
        public static string GetSelectId(Repeater repCtrl)
        {
            string strDeliId = "";
            try
            {
                for (int i = 0; i < repCtrl.Items.Count; i++)
                {
                    HtmlInputCheckBox objchk = (HtmlInputCheckBox)repCtrl.Items[i].FindControl("chkDelete");
                    if (objchk != null)
                    {
                        if (objchk.Checked)
                        {
                            strDeliId += objchk.Value + ",";
                        }
                    }
                }
                strDeliId = strDeliId.TrimEnd(',');
                if (strDeliId == "") throw new Exception("没有选择要操作的数据记录!");
            }
            catch (Exception ex)
            {
        ...
        /// <remarks>Create:2006.5.11 Modify:2006.5.11</remarks> 
        public static string GetTextContent(string strFileName)
        {
            if (!CheckFile(strFileName)) return "";
        ...
        /// </summary>
        /// <param name="strFullFileName">完整文件名</param>
        /// <remarks>Create:2006.5.9 Modify:2006.5.9</remarks> 
        public static bool CheckFile(string strFullFileName)
        {
        ...
        /// <summary>
        /// 检查目录是否存在,如果目录不存在,则新建目录

        /// </summary>
        /// <param name="strPath">目录名称</param>
        /// <remarks>Create:2006.5.9 Modify:2006.5.9</remarks> 
        public static void CheckFolderAndCreat(string strPath)
        {
        ...
        /// <param name="strTextField">text字段</param>
        /// <param name="strValueFiled">值字段</param>
        /// <param name="bHaveDefault">是否包含默认值</param>
        /// <remarks>2010-08-13 by David</remarks>
        public static void FillDropList(DataTable dt, DropDownList dd, string strTextField, string strValueFiled, bool bHaveDefault)
        {

            if (bHaveDefault)
            {
                dd.Items.Add(new ListItem("全部", "0"));
            }

            for (int i = 0; i < dt.Rows.Count; i++)
            {
                DataRow dr = dt.Rows[i];
                dd.Items.Add(new ListItem(dr[strTextField].ToString(), dr[strValueFiled].ToString()));
            }
            return;
        }
        ...
        /// <param name="strValueFiled">值字段</param>
        /// <param name="bHaveDefault">是否包含默认值</param>
        /// <remarks>2011-7-19  by aimee</remarks>
        public static void FillDropDownList(DataTable dt, DropDownList dd, string strTextField, string strValueFiled, bool bHaveDefault)
        {

            if (bHaveDefault)
            {
                dd.Items.Add(new ListItem("全部", ""));
            }
        ...
        /// <param name="strTextField">text字段</param>
        /// <param name="strValueFiled">值字段</param>
        /// <param name="strDefaultText">默认文字</param>
        /// <param name="strDefaultValue">默认值</param>
        /// <remarks>2011-7-16 by David</remarks>
        public static void FillDropList(DataTable dt, DropDownList dd, string strTextField, string strValueFiled, string strDefaultText, string strDefaultValue)
        {

            if (strDefaultText != "" && strDefaultValue != "")
            {
                dd.Items.Add(new ListItem(strDefaultText, strDefaultValue));
            }
        ...
        /// <param name="strValueFiled">值字段</param>
        /// <param name="strDefaultValue">默认值</param>
        /// <remarks>2011-6-23 by aimee</remarks>
        public static void FillDropList1(DataTable dt, DropDownList dd, string strTextField, string strValueFiled, string strDefaultValue)
        {
        ...
        /// 根据指定的值设置下拉列表的当前选定项
        /// </summary>
        /// <param name="dd">目标下拉列表</param>
        /// <param name="strValue">目标值</param>
        /// <remarks>2010-8-23 by David</remarks>
        public static void SeleteDropListByValue(DropDownList dd, string strValue)
        {
            for (int i = 0; i < dd.Items.Count; i++)
            {
                if (dd.Items[i].Value == strValue.Trim())
                {
        ...
        /// </summary>
        /// <param name="dd">目标下拉列表</param>
        /// <param name="strValue">目标文本</param>
        /// <remarks>2010-8-23 by David</remarks>
        public static void SeleteDropListByText(DropDownList dd, string strValue)
        {
            for (int i = 0; i < dd.Items.Count; i++)
            {
                if (dd.Items[i].Text == strValue.Trim())
                {
                    dd.Items[i].Selected = true;
                    break;
                }
        ...
        /// <returns></returns>
        /// <remarks>2011-7-16 by David</remarks>
        public static decimal GetDecimalFromString(string s)
        {
            ...
            try
            {
                sTemp = System.Text.RegularExpressions.Regex.Replace(s, @"[^\d^.]*", "");
                if (sTemp != "")
        ...
        /// <returns></returns>
        private string chineseShuziToAlabo(string strText)
        {
            return strText.Replace("一", "1").Replace("二", "2").Replace("三", "3").Replace("四", "4").Replace("五", "5").Replace("六", "6").Replace("七", "7").Replace("八", "8").Replace("九", "9").Replace("十", "10");
        }
        ///<summary>
        ///将字符串转换为DateTime类型
        ///</summary>
        public static DateTime stringToDatetime(string stringSource)
        {
            try
            {
                return Convert.ToDateTime(stringSource);
            }
            catch (Exception)
            {
        ...
        /// <summary>
        /// 根据指定的XSLT模版，将XML数据转化为HTML格式内容返回
        /// </summary>
        /// <param name="strXml">XML数据字符串/param>
        /// <param name="strXslt">XSLT样式完整路径</param>
        /// <returns></returns>
        /// <remarks>2011-7-16 by David</remarks>
        public static string GetHtmlByXslt(string strXml, string strXsltPath)
        {
            string htmlContent = "";
            try
            {
                ...
                if (strXml != "")
                {
                    ...
                    htmlContent = Encoding.GetEncoding("utf-8").GetString(stream.ToArray());
                    stream.Close();
                    stream.Dispose();
                }
            }
        ...
        /// </summary>
        /// <param name="n">一般为根节点</param>
        /// <param name="strDataType">数据类型,可用于扩展修改多种数据</param>
        /// <param name="strNodeName">目标节点名称</param>
        /// <param name="strFormat">显示的格式,暂时只使用一日期格式 ,可以扩展多种格式</param>
        /// <remarks>2011-7-16 by David</remarks>
        public static void SetNodeFormate(XmlNode n, string strDataType, string strNodeName, string strFormat)
        {
            if (n.HasChildNodes)
            {
                foreach (XmlNode nd in n.ChildNodes) //递归修改所有节点
                {
                    SetNodeFormate(nd, strDataType, strNodeName, strFormat);
                }
            }
            else if (n.ParentNode.Name == strNodeName) //此处的判断逻辑有待于进一步完善
            {
                SetNodeFormateValue(n.ParentNode, strDataType, strFormat);
            }
            return;
        }
        /// <summary>
        ...
        /// <remarks>2011-7-16 by David</remarks>
        private static void SetNodeFormateValue(XmlNode n, string strDataType, string strFormat)
        {
            switch (strDataType.ToLower()) //暂时只添加了对日期的处理方法
            {
                case "datetime":
                    n.InnerText = GetDateTimeFromString(n.InnerText).ToString(strFormat, System.Globalization.DateTimeFormatInfo.InvariantInfo);
                    break;
                case "int":
                    break;
            }
        ...
        /// <returns></returns>
        /// <remarks>2011-7-4 by David</remarks>
        ...
        /// 
        /// </summary>
        /// <param name="strEmailAddresses"></param>
        /// <returns></returns>
        /// <remarks>2011-8-15 by David</remarks>
        public static string DealManyEmailAddress(string strEmailAddresses)
        {
            Regex reg = new Regex("^\\s*([A-Za-z0-9_-]+(\\.\\w+)*@(\\w+\\.)+\\w{2,5})\\s*$");
            StringBuilder resultAddress = new StringBuilder();
            strEmailAddresses = strEmailAddresses.Replace("，", ",");
            string[] addressArray = strEmailAddresses.Split(',');
            foreach (string address in addressArray)
            {
                string singleAddress = address.Trim();
                if (!string.IsNullOrEmpty(singleAddress))
                {
                    if (reg.IsMatch(singleAddress))
                        resultAddress.Append(singleAddress + ",");
                    else
                        return "";
                }
            }
            return resultAddress.ToString().TrimEnd(',');
        ...
        /// <returns></returns>
        /// <remarks>2011-8-9 by David</remarks>
        public static string NoHTML(string Htmlstring) //
        {
            //删除脚本
            //Htmlstring = Regex.Replace(Htmlstring, @"<script[^>]*?>.*?</script>", "", RegexOptions.IgnoreCase); //只替换JavaScript标签
            Htmlstring = Regex.Replace(Htmlstring, @"<script[^>]*?>(.|\r|\n)*?</script>", "", RegexOptions.IgnoreCase); //包括Javascript代码段
        ...
        /// <param name="strfldName">主键字段名</param>
        /// PX 排序 非0值为降序
        /// <remarks>Modify 2011-7-16 by David</remarks>
        /// <returns>返回两张表,第一表存储分页数据,第二张表存储页面记录总数,字段名称为Total</returns>
        public static DataSet GetListCommon(int PageSize, int PageIndex, string strWhere, string sTableName, string strfldName, int PX)
        {
            ...
            parameters[1].Value = strfldName;
            parameters[2].Value = PageSize;
            parameters[3].Value = PageIndex;
            parameters[4].Value = 1;
            parameters[5].Value = PX;
            parameters[6].Value = strWhere;
            return Maticsoft.DBUtility.DbHelperSQL.RunProcedure("UP_GetRecordByPage", parameters, "ds");
        }
        /// <summary>
        /// 验证输入内容是否合法  by David 6-30
        /// </summary>
        ...
        public static string GetQueryStringSafe(string pStr)
        {
            ...
            {
                pStr = pStr.ToLower();
                string strSQLin = ConfigurationManager.AppSettings["stringField"];
                //string strSQLin = "'|and|--|exec|insert|select|delete|update|count|*|%|chr|mid|master|truncate|char|declare|;|&|%20|==|>|<";
                string[] strSQLinGroup = strSQLin.Split(new char[] { '|' }, 23);
                for (int i = 0; i < strSQLinGroup.Length; i++)
                {
                    if (pStr.IndexOf(strSQLinGroup[i]) >= 0)//匹配成功，说明含有非法字符
                    {
                        IsValue += strSQLinGroup[i];
                        break;
                    }
                }
            }
        ...
        /// <summary>
        /// 验证参数是否含有非法字符
        /// 如果是空值也返回false即当做没有非法字符处理
        /// </summary>
        /// <param name="parameter">需要验证的参数</param>
        /// <returns>true如果该参数含有非法字符否则返回false</returns>
        /// <remarks>非法字符包括'|%|like|or|and|exec|insert|select|delete|update|count|chr|mid|master|
        ///                                     truncate|char|declare|drop|drop table|create|creat table|&lt;script|&lt;iframe|&lt;
        /// </remarks>
        public static bool CheckQueryString(string parameter)
        {
            bool flag = false;
            if (!string.IsNullOrEmpty(parameter))
            {
                //获得非法字符集
                string strSQLin = ConfigurationManager.AppSettings["stringField"];
                //对非法字符集进行分割以得到每一个非法字符
                string[] strSQLinGroup = strSQLin.Split(new char[] { '|' }, 23);
                //循环进行判断，如果参数中包含非法字符，则返回false
                for (int i = 0; i < strSQLinGroup.Length; i++)
                {
                    if (parameter.ToLower().IndexOf(strSQLinGroup[i]) >= 0)
                    {
                        flag = true;
                        break;
                    }
        ...
        #region Cookie操作 Alex 2011-09-07添加
        /// 创建COOKIE对象并赋Value值
        /// <summary>
        ...
        #region DES加密相关操作 Alex 2011-09-29添加
        ...
        /******************************************************************
          * 函数名称：WriteFile
          * 功能说明：写文件,不会覆盖掉以前的内容,而是追加,每次写一行
          * 参　　数：Path:文件路径,FileName文件名,Strings:文本内容
          * 说　　明 : 该方法是以gb2312编码形式写入文件
          * 调用示列：
          *                 string Path = Server.MapPath("test.aspx");       
          *                 string Strings = "你好.Hello World";
          *                 JobSearch.Common.Utils.WriteFile(Path,Strings);
         ******************************************************************/
        /// <summary>
        /// 写文件
        /// </summary>
        ...
        public static void WriteFile(string Path, string FileName, string Strings)
        {
            ...
            if (File.GetAttributes(Path + FileName) == FileAttributes.ReadOnly)
            {
                File.SetAttributes(Path + FileName, FileAttributes.Normal);
            }
            System.IO.StreamWriter f2 = new System.IO.StreamWriter(Path + FileName, true, System.Text.Encoding.GetEncoding("gb2312"));
            lock (f2)
            {
                string temp = "";
